...
bsize = lambdautils.compute_batch_size(
...
write_job_config(job_id, job_bucket, n_mappers,
...
while True:
    job_keys = s3_client.list_objects(
        Bucket=job_bucket, Prefix=job_id)["Contents"]
    ...
    if job_id + "/result" in keys:
        print("job done")
        reducer_lambda_time += float(s3.Object(job_bucket,
...
# PUT, COPY, POST, LIST 요청 비용 Request 0.005 USD / request 1000
s3_put_cost = len(job_keys) * 0.005/1000

total_s3_get_ops += len(job_keys)
# GET, SELECT, etc 요청 비용 Request 0.0004 USD / request 1000
